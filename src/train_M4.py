...
def val_decision_cost(
    ...
    d_miss: float,
    device: torch.device,
    val_availabilities: list[np.ndarray],
) -> float:
    """
    Mean scheduling cost over multiple val availability realizations on grades 1–4 val set.
    Averaging over k realizations reduces checkpoint-selection variance when constraints are binding.
    """
    model.eval()
    ...
    costs = []
    for avail in val_availabilities:
        z_np  = solve_multislot_availability(scores_np, K_list, avail,
                                             delay=delay.cpu().tolist(), d_miss=d_miss, beta=beta)
        z     = torch.tensor(z_np, dtype=alpha.dtype)
        costs.append(scheduling_cost_multislot(z, labels, alpha.cpu(), beta, delay.cpu(), d_miss).item())
...
def train_M4(
    ...
    _sigma = sigma if sigma is not None else CONFIG["sigma"]
    logger.info("=== M4  seed=%d  sev_frac=%.2f  sigma=%.2f  device=%s ===",
                seed, severity_fraction, _sigma, device)
    ...
    # Multiple val realizations reduce checkpoint-selection variance when constraints are binding.
    avail_dir      = Path(avail_dir)
    val_avail_seed = CONFIG["availability_seed_val"]
    val_availabilities: list[np.ndarray] = []
    for i in range(n_val_avail):
        path = avail_dir / f"val_availability_seed{val_avail_seed + i}.npy"
        val_availabilities.append(np.load(path))
        logger.info("Loaded val availability [%d/%d]: shape=%s, path=%s",
                    i + 1, n_val_avail, val_availabilities[-1].shape, path)
    ...
    if stage3_only:
        # ── Stage 3-only mode: skip Stage 2, load provided checkpoint ────────
        s2_src = Path(stage2_ckpt) if stage2_ckpt else model_dir / f"M4_stage2_seed{seed}.pt"
        if not s2_src.exists():
            raise FileNotFoundError(f"Stage 2 checkpoint not found: {s2_src}")
        model.load_state_dict(torch.load(s2_src, map_location=device, weights_only=True))
        logger.info("Stage3-only: loaded Stage 2 checkpoint → %s", s2_src)
        best_val_cost = val_decision_cost(model, val_loader, alpha, beta, CONFIG["K_frac_list"],
                                         delay, d_miss, device, val_availabilities)
        logger.info("Stage 2 baseline val cost (loaded ckpt): %.4f", best_val_cost)
        sigma_tag = f"sigma{_sigma:.1f}_"
    else:
        # ── Full run: Stage 2 + Stage 3 ───────────────────────────────────────
        ...
        if not m1_ckpt.exists():
            raise FileNotFoundError(f"M1 checkpoint not found: {m1_ckpt}\nRun train_M1.py first.")
        ...
        stage2_ckpt_path = model_dir / f"M4_stage2_seed{seed}.pt"
        ...
        for epoch in range(CONFIG["epochs_stage2"]):
            ...
            if val_cost < best_val_cost:
                ...
                patience_ctr  = 0
                torch.save(model.state_dict(), stage2_ckpt_path)
        ...
        model.load_state_dict(torch.load(stage2_ckpt_path, map_location=device, weights_only=True))
        sigma_tag = ""
    ...
    # Stage 3 overwrites it only when val cost improves.
    final_ckpt = model_dir / f"M4_{sigma_tag}seed{seed}.pt"
    if stage3_only:
        s2_src = Path(stage2_ckpt) if stage2_ckpt else model_dir / f"M4_stage2_seed{seed}.pt"
        shutil.copy2(s2_src, final_ckpt)
    else:
        shutil.copy2(stage2_ckpt_path, final_ckpt)
    ...
    logger.info(
        ...
        n3_trainable, CONFIG["lr_stage3"], CONFIG["epochs_stage3"], _sigma, CONFIG["M"],
    )
    ...
    pred_csv = output_dir / f"M4_{sigma_tag}seed{seed}.csv"
    ...
    metrics = evaluate(pred_csv, alpha=CONFIG["alpha"], beta=CONFIG["beta"],
                       ...
                       severity_only=True)
    metrics_path = output_dir / f"M4_{sigma_tag}seed{seed}_metrics.json"
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--n-val-avail",  type=int,   default=1,
                   help="Number of val availability realizations to average for checkpoint selection")
    p.add_argument("--sigma",        type=float, default=None,
                   help="Override CONFIG['sigma'] for Stage 3 perturbation std")
    p.add_argument("--stage3-only",  action="store_true",
                   help="Skip Stage 2; load --stage2-ckpt and run Stage 3 only")
    p.add_argument("--stage2-ckpt",  default=None,
                   help="Path to Stage 2 checkpoint (used with --stage3-only)")
