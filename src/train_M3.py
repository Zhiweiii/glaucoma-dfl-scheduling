...
def val_decision_cost(
    ...
    """
    Light-touch validation criterion: mean scheduling cost over multiple val
    availability realizations on grades 1–4 val set.

    Averaging over k realizations reduces the variance of the checkpoint
    selection signal, preventing the selection from overfitting to a single
    availability instance (which becomes problematic when constraints are binding).
    """
    model.eval()
...
def train_M3(
    ...
    # ── Training: CE on trunk+severity_head, checkpoint by val scheduling cost ──
    ...
    logger.info(
        "=== M3 Training: backbone[9+] lr=%.2e | trunk+sev_head lr=%.2e | epochs=%d ===",
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--n-val-avail", type=int, default=1,
                   help="Number of val availability realizations to average for checkpoint selection "
                        "(seeds val_avail_seed, val_avail_seed+1, ...). Requires those .npy files.")
