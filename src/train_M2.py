"""
Train M2: Baseline severity head.
...
Phase 2 of the feature-freezing decomposition:
  ...
  - Checkpoint selection: validation CE loss (decision-blind baseline)

Ablation role: compared to M3 (same architecture, decision-aware selection) and
M4 (same init, adds DFL fine-tuning) to isolate each contribution independently.

Triage score: α̂_i = Σ_k α_k · softmax(severity_logits)_ik  ∈ [0, 10]

Output:
  models/M2_seed{seed}.pt               — best checkpoint (lowest val CE)
  results/M2_seed{seed}.csv             — prediction CSV for evaluate.py
  results/M2_seed{seed}_metrics.json    — scheduling metrics (auto-evaluated)

Usage:
    python src/train_M2.py --seed 42
    python src/train_M2.py --seed 42 --severity-fraction 0.25
"""
...
def val_decision_cost(
    ...
    """
    Scheduling cost on grades 1–4 val set. Logged for diagnostics; not used for
    M2 checkpoint selection. val_availability is sized for grades 1–4 (generated
    by generate_availability.py with grade ≥ 1 filter — no pre-filtering needed here).
    """
    model.eval()
...
def val_severity_ce(
    ...
    """Severity CE on val set — used for M2 checkpoint selection."""
    model.eval()
    total, n = 0.0, 0
    with torch.no_grad():
        for imgs, _, sev_labels, has_severity, _ in loader:
...
def train_M2(
    ...
    """
    Train M2 and write results/M2_seed{seed}.csv.
    ...
    logger.info("=== M2  seed=%d  sev_frac=%.2f  device=%s ===",
    ...
    logger.info("Best val CE: %.4f", best_val_ce)
    ...
    pred_csv = output_dir / f"M2_seed{seed}.csv"
    ...
    metrics = evaluate(pred_csv, alpha=CONFIG["alpha"], beta=CONFIG["beta"],
                       ...
                       severity_only=True)
    metrics_path = output_dir / f"M2_seed{seed}_metrics.json"
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train M2 (baseline severity head) and write prediction CSV.",
...
if __name__ == "__main__":
    ...
    print(f"Next: python src/evaluate.py --predictions {pred_csv} "
          f"--output results/M2_seed{args.seed}_metrics.json")
